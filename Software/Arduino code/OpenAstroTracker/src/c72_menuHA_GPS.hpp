...
bool gpsAqcuisitionComplete()
{
    while (GPS_SERIAL_PORT.available())
    {
        ...
        if (gps.encode(gpsChar))
        {
            ...
            {
                LOGV2(DEBUG_INFO, "Found GPS location. Age is %d secs", gps.location.age() / 1000);
                LOGV4(DEBUG_INFO, "GPS UTC is %dh%dm%ds", gps.time.hour(), gps.time.minute(), gps.time.second());
...
bool processHAKeys()
{
    ...
    if (haState == STARTING_GPS)
    {
        ...
        {
            LOGV1(DEBUG_INFO, "HA: GPS acquired");
            GPS_SERIAL_PORT.end();
            haState = SHOWING_HA_SYNC;
            if (startupState == StartupWaitForHACompletion)
            {
                LOGV1(DEBUG_INFO, "HA: We were in startup, so confirm HA");
    ...
    {
        waitForRelease = true;
        LOGV3(DEBUG_INFO, "HA: Key %d was pressed in state %d", key, haState);
        ...
        if (key == btnRIGHT)
        {
            LOGV1(DEBUG_INFO, "HA: Right Key was pressed");
            if (haState == STARTING_GPS)
            {
                LOGV1(DEBUG_INFO, "HA: In GPS Start mode, switching to manual");
                GPS_SERIAL_PORT.end();
                haState = SHOWING_HA_SYNC;
            }
            else if (startupState == StartupWaitForHACompletion)
            {
                LOGV1(DEBUG_INFO, "HA: In Startup, not in GPS Start mode, leaving");
                startupState = StartupHAConfirmed;
                inStartup = true;
            }
            else
            {
                LOGV1(DEBUG_INFO, "HA: leaving HA");
